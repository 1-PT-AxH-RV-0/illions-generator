...
1. 处理键表示 c3g 映射表的元素
   1. 构建第三类分隔符
      ...
      3. 将第二层级分界线拼接在第三层级分界线之前，构成**第三类分隔符**
      4. 按上述步骤构建出该 c3g 映射表中的所有的第三类分隔符
   2. 构建第二类分隔符
      1. 将该 c3g 映射表中的所有的第三类分隔符按 i<sub>c3g</sub> 的大小降序排序，构成**未完成的第二类分隔符**（记为 u<sub>c2s</sub>）
      2. 如果 u<sub>c2s</sub> 的最后一个第三类分隔符的 i<sub>c3g</sub> 不为 0，则要将 u<sub>c2s</sub> 末尾的一个元音改成“o”
      3. 在 u<sub>c2s</sub> 的开头拼接上第 v<sub>c2g</sub> 个前缀模式的第二层级分界线，构成**第二类分隔符**，将其记为 s<sub>c2</sub>
2. 处理键表示第二层级分界线索引的元素
   - 按[第二层级的处理方式](#第二层级)处理即可，但是**只用处理到得到第一类分隔符这个步骤**。注意，**对于第三层级来说，这里得到的第一类分隔符是不完整的**，所以将其记为 p<sub>c1s</sub>
3. 拼接出最终结果
   1. 将所有的 s<sub>c2</sub> 用“-”连接，构成**可能不完整的第一类分隔符**，将其记为 t<sub>c1s</sub>
   2. 如果有 p<sub>c1s</sub> 的话，将它拼接在 t<sub>c1s</sub> 之后，中间用“-”连接，构成**第一类分隔符**，否则**直接由 t<sub>c1s</sub> 构成第一类分隔符**，记为 s<sub>c1</sub>
