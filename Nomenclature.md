...
1. 处理键表示 c3g 映射表的元素
   ...
   2. 构建文字第二类组
      1. 将该 c3g 映射表中的所有的文字第三类组按 i<sub>c3g</sub> 的大小降序排序，构成**未完成的文字第二类组**（记为 u<sub>c2s</sub>）
      2. 如果 u<sub>c2s</sub> 的最后一个文字第三类组的 i<sub>c3g</sub> 不为 0，则要将 u<sub>c2s</sub> 末尾的一个元音改成“o”
      3. 在 u<sub>c2s</sub> 的开头拼接上第 v<sub>c2g</sub> 个前缀模式的第二层级分界线，构成**文字第二类组**，将其记为 s<sub>c2</sub>
2. 处理键表示第二层级分界线索引的元素
   - 按[第二层级的处理方式](#第二层级)处理即可，但是**只用处理到得到文字第一类组这个步骤**。注意，**对于第三层级来说，这里得到的文字第一类组是不完整的**，所以将其记为 p<sub>c1s</sub>
3. 拼接出最终结果
   1. 将所有的 s<sub>c2</sub> 用“-”连接，构成**可能不完整的文字第一类组**，将其记为 t<sub>c1s</sub>
   2. 如果有 p<sub>c1s</sub> 的话，将它拼接在 t<sub>c1s</sub> 之后，中间用“-”连接，构成**文字第一类组**，否则**直接由 t<sub>c1s</sub> 构成文字第一类组**，记为 s<sub>c1</sub>
   3. 如果 s<sub>c1</sub> 不以第一类分界线结尾，则去掉末尾的一个元音
   4. 在 s<sub>c1</sub> 之后拼接上“illion”，构成最终结果
